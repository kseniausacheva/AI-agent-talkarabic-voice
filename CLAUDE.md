...
- **Спринт 6 (новые инструменты — мини-CRM, 2026-06-15):** свой инструментарий
  ...
  - LLM остаётся MiniMax M3, НО обсуждается переход на OpenAI API (gpt-4o-mini —
    быстрее/качественнее; подписка ChatGPT ≠ API, нужен отдельный ключ). Переход =
    смена 3 env (base_url/key/model), код OpenAI-совместим.
...
- **Обновить прод:** `git push` в `main` → Vercel пересобирает фронт автоматически;
  **бэкенд тоже авто-деплоится при push в main** (Coolify webhook, пересборка
  ~12-15 мин из-за torch — наблюдалось 2026-06-14/15). Кнопка **Redeploy** в Coolify —
  для ручного повтора; **Restart** — только перезапуск контейнера с новыми env
  (~30 сек, без пересборки). Колонки-миграции (`deal_json` и т.п.) накатываются
  автоматически в `init_db()` при старте контейнера.
