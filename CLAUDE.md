...
> 1. **Ускорить формирование чеклиста** — финальный шаг «миллион лет», слишком медленно.
> 2. **Премиум-редизайн под бренд** — БЕЛЫЙ фон, точные цвета школы (красный `#fb3501`,
>    голубой `#43abd0`), эффект «ВАУ»/VIP, возможно Three.js. Текущая тёмная тема НЕ подошла.
> Детали, гипотезы и где смотреть — в `docs/NEXT-STEPS.md`.
